...
class StateTests: XCTestCase {
  ...
  func testStateRange() {
    ...
    let normalState = State(offset: -200, configuration: testConfiguration)
    XCTAssertEqual(normalState.stateRange(), .normalExpanded)

    let expandedState = State(offset: -300, configuration: testConfiguration)
    XCTAssertEqual(expandedState.stateRange(), .normalExpanded)
  }

  func testSetOffset() {
    let state = State(offset: -100, configuration: testConfiguration)
  ...
  func testHeight() {
    let state = State(offset: -50, configuration: testConfiguration)
  ...
  func testTransitionProgress() {
    ...
    let semiExpandedState = State(offset: -240, configuration: testConfiguration)
    XCTAssertEqual(semiExpandedState.transitionProgress(), 1.4)

    let compactState = State(offset: -100, configuration: testConfiguration)
    XCTAssertEqual(compactState.transitionProgress(), 0)

    let semiNormalState = State(offset: -170, configuration: testConfiguration)
